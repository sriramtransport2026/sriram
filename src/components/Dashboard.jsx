...
export function Dashboard({ 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-200/70">
        <div className="flex flex-wrap items-center gap-2.5 text-xs text-slate-500 font-medium">
          ...
          {currentUser?.assigned_company_name && currentUser.assigned_company_name !== 'All Companies' && (
            <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-blue-50 border border-blue-200 text-blue-900 text-[11px] font-bold shadow-sm ml-2">
              <Building2 className="w-3.5 h-3.5 text-blue-700 shrink-0" />
              <span>Dedicated Company Scope: <strong className="text-brand-navy underline">{currentUser.assigned_company_name}</strong></span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {currentUser && (
            <div className="flex items-center space-x-3 bg-white px-3.5 py-1.5 rounded-full border border-slate-200/80 shadow-sm">
              <div className="w-6 h-6 rounded-full bg-brand-navy text-brand-gold text-[11px] font-black flex items-center justify-center uppercase shadow-inner">
              ...
              <button
                ...
                className="flex items-center space-x-1 text-xs font-bold text-rose-600 hover:text-rose-700 hover:bg-rose-50 px-2.5 py-1 rounded-full transition-colors cursor-pointer"
              >
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-brand-navy-dark via-brand-navy to-brand-navy-subtle rounded-3xl p-8 sm:p-10 shadow-soft-lg text-white border border-brand-navy-light/50">
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
          
          {/* Logo & Brand Details */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            {/* Logo Badge Container */}
            <div className="bg-white p-3.5 sm:p-4 rounded-2xl shadow-xl border border-white/20 shrink-0 flex items-center justify-center">
              <img 
                src={logoImg} 
            ...
            <div className="space-y-2.5">
              <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-brand-gold/15 border border-brand-gold/30 text-brand-gold text-xs font-bold tracking-wider uppercase">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-3 py-1 rounded-full bg-white/10 text-white font-mono text-xs font-black border border-white/20 shadow-sm flex items-center space-x-1.5">
                ...
                <span className="px-3 py-1 rounded-full bg-white/10 text-slate-200 font-mono text-xs font-bold border border-white/20 shadow-sm flex items-center space-x-1.5">
                ...
                {isUserAdmin && (
                  <button
                    ...
                    className="px-3 py-1 rounded-full bg-brand-gold hover:bg-brand-gold-light text-brand-navy font-black text-xs flex items-center space-x-1.5 shadow-md transition cursor-pointer"
                  >
                    <ArrowLeftRight className="w-3.5 h-3.5" />
                    <span>Switch Firm / GST</span>
                  </button>
                )}
              </div>

              {/* Logo Sub-tagline */}
              <div className="flex items-center space-x-2 text-xs font-extrabold tracking-widest text-brand-gold uppercase pt-0.5">
              ...
              <div className="text-xs text-slate-300 font-medium flex flex-wrap items-center gap-3 pt-0.5">
          ...
          <div className="grid grid-cols-3 gap-3 shrink-0 bg-white/5 backdrop-blur-md p-4 rounded-2xl border border-white/10 text-center">
            <div className="px-3">
              <p className="text-[10px] font-bold text-brand-gold-light uppercase tracking-wider">Booked</p>
              <div className="mt-1 flex items-center justify-center space-x-1 text-brand-gold font-black text-xl font-display">
                <Clock className="w-4 h-4 inline" />
                <span>{metrics.bookedCount}</span>
              </div>
            </div>
            <div className="px-3 border-x border-white/10">
              <p className="text-[10px] font-bold text-blue-300 uppercase tracking-wider">In Transit</p>
              <div className="mt-1 flex items-center justify-center space-x-1 text-blue-400 font-black text-xl font-display">
                <Truck className="w-4 h-4 inline" />
                <span>{metrics.inTransitCount}</span>
              </div>
            </div>
            <div className="px-3">
              <p className="text-[10px] font-bold text-emerald-300 uppercase tracking-wider">Completed</p>
              <div className="mt-1 flex items-center justify-center space-x-1 text-brand-green font-black text-xl font-display">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-black text-brand-navy tracking-tight font-display">
              Operations Hub
            </h2>
          ...
          <span className="text-xs font-bold px-3 py-1 bg-white border border-slate-200 rounded-full text-brand-navy shadow-sm">
            7 Active Modules
          </span>
        </div>
          ...
          {modules.map((mod) => {
            ...
            return (
              <div
                ...
                className={`group relative bg-white rounded-2xl p-6 shadow-soft hover:shadow-soft-lg transition-all duration-200 cursor-pointer border border-slate-200/80 ${mod.accentColor} flex flex-col justify-between`}
              >
                <div>
                  {/* Card Header: Icon & Live Badge */}
                  <div className="flex items-start justify-between mb-4">
                    <div className={`p-3.5 rounded-2xl ${mod.iconBg} transition-transform duration-200 group-hover:scale-105`}>
                      <Icon className="w-6 h-6" />
                    </div>
                    <span className={`text-[11px] font-bold px-3 py-1 rounded-full border ${mod.badgeColor}`}>
                      {mod.badge}
                    </span>
                  </div>

                  {/* Title & Description */}
                  <h3 className="text-lg font-bold text-brand-navy group-hover:text-brand-navy-light transition mb-1 font-display">
                    {mod.title}
                  </h3>
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <div className="w-8 h-8 rounded-full bg-slate-50 group-hover:bg-brand-navy group-hover:text-white flex items-center justify-center text-slate-400 transition-colors">
